...
def parse_args():
    ...
    parser.add_argument('-config', help='path to model configuration file',default=r"E:\_Flood_Inundation_Mapping\PycharmProjects\pythonProject2\sen1floods11_Prithvi_100M.py")
    parser.add_argument('-ckpt', help='path to model checkpoint',default=r"E:\_Flood_Inundation_Mapping\Code\hls-foundation-os-main\FloodMapping\HLSSegmentResume-450\work_dirs\epoch_320.pth")
    parser.add_argument('-input', help='path to input images folder for inference',default=r"D:\_FloodAreas\2BardiyaIndia221010\1_TIFF\\")
    parser.add_argument('-output', help='path to save output image', default=r"D:\_FloodAreas\2_Bardiya_320\3_TIFFSegment\\")
...
def inference_on_file(model, target_image, output_image, custom_test_pipeline):
    ...
    try:
        ...
        print("Output has shape: " + str(result[0][0].shape))
        ...
        result[0][0] = np.where(mask == 1, -1, result[0][0])
        ...
        meta["nodata"] = -1
        print('Saving output...')
        write_tiff(result[0][0], output_image, meta)
        et = time.time()
...
def inference_on_files(config_path, ckpt, input_type, input_path, output_path, bands):
    ...
    for i, target_image in enumerate(target_images):
        ...
        output_image = output_path+target_image.split("\\")[-1].replace('.' + input_type, '-pred.'+input_type)
